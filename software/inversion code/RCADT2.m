...
T2minp = -3.301; T2maxp = 1; % define the range of the inversion. T2 from 1ms to 1s.
...
if(strcmp(Params.experiment,'DT2_PGSTE'))
...
elseif(strcmp(Params.experiment,'DT2v2'))
    ...
    kernfac = 1;
    elseif(strcmp(Params.experiment,'DT2'))
    display('Processing data from unipolar stimulated echo D-T2 sequence...');
    kernfac = 1;
else
...
% GradArray = 0.95*GradArray;     % Gradient calibration on October 12th 2011
 GradArray = 1.05*GradArray;     % Gradient calibration on 20130721,Zhixiang Luo
...
% end
 ...
 axis([0 1e6 1e-03 10])% add this line to have all scale fixed;
 pcs(1) 
saveas(gcf,'Dlinfits');
...
axis([1 1e7 0.0 4.5])% add this line to have all scale fixed;
 saveas(gcf,'Dfits');
...
if 0
    ...
    figure('Color',[1 1 1])
    disp(['theAlphalist = ', num2str(theAlphalist)]);
    FLIPlot2dFullDT2(D,T2,FEstlist{theIndex}.f);
