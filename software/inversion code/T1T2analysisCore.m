% data analysis for generic data
% 
function T1T2analysis(tau1,tau2,data2d)
...
plot(mean(datadiff))
hold on
plot(std(datadiff) ./ sqrt(size(Fit,1)) ,'r-')
plot(std(datadiff)  ,'r-')
hold off
set(gca, 'XTick', [1e-3 1e-2 1e-1 1 10]);
xlabel ('\tau_2 second')
...
plot(tau2,mean(datadiff,2),'-',tau2,std(datadiff(:,1:end-1),1,2))
