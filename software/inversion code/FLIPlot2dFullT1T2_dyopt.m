function  FLIPlot2dFullT1T2_dyopt(T1, T2, FEst, inCM)  
%
...
% 
    ...
    hold on
    mycontourlines = [.1 .5 .9]*max(max(FEst));
    contour(x, y, FEst(1:length(y), 1:length(x)),mycontourlines,'m-')
    hold off
	%axis square
	h = gca;
	set(h, 'XScale', 'log', 'YScale', 'log')
	xlabel('T_2 (ms)', 'FontSize', 9)
	ylabel('T_1 (ms)', 'FontSize', 9)
	set(h, 'FontSize', 10);
	set(h, 'XTickMode', 'Manual');
	set(h, 'XTick', [1e-3 1e-2 1e-1 1 10]);
	set(h, 'YTickMode', 'Manual');
	set(h, 'YTick', [1e-3 1e-2 1e-1 1 10]);
