...
[freq,p] = magnet_temp(index2, Temp);
infreq2 = freq*1e6*2 %calculated larmor freq

infreq = 45235000/2;

%% infreq = 23.4268*1000000 + 4e4 - 10600; %in hertz
folder = 'C:\Users\ytang12\OneDrive - Schlumberger\01_Working folder\ASIC in oven';
filename = 'NMRdata2.mat';
...
temp = 2*(infreq -f(index))

infreq = temp/2
save_data(myNMR,infreq,y,folder,filename,0)
%% nutation exp 
% 2/14/2018 Ray Tang
...
elseif ws == 1
    ...
    TD = 4000; % run into a bug when TD > 4500. Need investigation. 1/30/2018
end
...
% T2 inversion
TE = (TE + t180)/1e6;
[T2,T2dist,tau2] = T2analysismicroNMR(y2',TE,dummyecho,TD)
%% for echoshape = 1
TE = 3 %in ms
t180 = 80e-3 % in ms
...
dummyscan = 0;
p = pindex;
vd = [5e3,5e4,1e5,5e5,1e6,2e6,3e6,5e6,8e6,1e7,1.5e7]; %variable delay for T1 encoding
%vd = [10e6];
...
T1T2analysismicroNMR(y,TE,dummyecho,TD,vd)